...
(function (majo) {
	...
	var TextMeme = function (id) {
  	...
  	ctxcache.drawImage(this.ctx.canvas,0,0);
		...
		this.getStyle = function () {
			var style = ["text-transform: uppercase;", "display: block;", "text-align: center;", "word-wrap: break-word;", "z-index: 1;", "font:" + this.getFont().size + "px Impact, Arial Black;", "font-weight: bold;", "color:" + this.getFont().color + ";", "-webkit-text-fill-color: white;", "-webkit-text-stroke-width: 3px;", "-webkit-text-stroke-color: black;", "text-stroke-width: 1px;", "text-stroke-color: black;", "width:" + this.ele.offsetWidth + "px", "height:" + this.ele.offsetHeight + "px"];
			return style;
		};
	...
	var TextStandard = function (id) {
		...
		this.getStyle = function () {
			return ["text-transform: uppercase;", "display: block;", "text-align: center;", "word-wrap: break-word;", "z-index: 1;", "font: " + this.getFont().size + "px Impact, Arial Black;", "color:" + this.getFont().color + ";", "width:" + this.ele.offsetWidth + "px", "height:" + this.ele.offsetHeight + "px"];
		};
		/*this.draw = function(){
  	if(color.fillStyle){
  		ctx.lineWidth = 6;
...
var ImagenMeme = React.createClass({
	handleClick: function (evt) {
		...
		if (MajoCreator.ableNew()) {
			...
			var urlimage = evt.target.src;
			var altext = evt.target.alt;

			img.onload = function () {
				ct.drawImage(img, 0, 0, 700, 500);
				var src = c.toDataURL();
				MajoCreator.newImagenground(src, index);
	...
	render: function () {
		return React.createElement("img", { src: this.props.data.img, tabIndex: this.props.data.id, title: this.props.data.alt, alt: this.props.data.alt, onClick: this.handleClick });
	}
});
		...
		return React.createElement(
			...
			React.createElement(
				...
				React.createElement("input", { type: "text",
					placeholder: "Search image or put url..",
	...
	searchBycategory: function (event) {
		...
		if (valor.trim().length > 0) {
			$.ajax({
				...
				success: function (data) {
					var dt = JSON.parse(data);
					self.handleUserSearch(dt, valor);
				},
				beforeSend: function () {},
				complete: function () {}
			});
		...
		return React.createElement(
			...
			React.createElement(
				...
				React.createElement(
					...
					React.createElement("svg", { id: "sgvcanvas" }),
					React.createElement(
						"div",
						{ id: "contenttext" },
